...
OFFLINE (batch)                                        ONLINE (serves users)
  ...
  Jikan     >  ingest -> warehouse -> train -> publish   read-only artifact + caching
  StackEx  /   (cache)   (DuckDB/    (models)  (serving   static React SPA on a CDN
                          Parquet)              artifact)
```
...
src/anime_stackviz/
  ...
  storage/             # Storage interface (local or object store)
